...
Date: 2026-06-27
Status: Revised and approved for specification by the user's Batch 0 decision

## 1. Objective

Add permanent ingestion-batch grouping to the classified article library.

- The 18 currently classified articles become the baseline cohort, `Sınıflandırma 00` (`classificationBatch: 0`).
- The first future run of `prompts/01-classify-and-order-articles.md` that imports at least one new, non-duplicate article creates `Sınıflandırma 01`; later successful imports continue with 02, 03, 04, and so on.
...
### C. Store a non-negative batch number on every article (selected)

Add `classification_batch` to frontmatter and `classificationBatch` to catalog records. Batch 0 is the immutable baseline for articles already inside the classified library when batch tracking is introduced. The next batch is derived from the maximum existing value. This is explicit, statically available to the UI, easy to validate, and remains stable across Git operations.
...
Batch values are stored as numbers without leading zeroes. Two-digit padding is a presentation rule only: numeric `0` renders as `00`, numeric `1` as `01`, and numeric `12` as `12`.

### Invariants

- Every article has one non-negative integer batch.
- Batch 0 is reserved for the 18 articles already in the classified library during this migration; no future import can receive Batch 0.
- Existing article batches never change during normal ingestion, body maintenance, or explicit reclassification.
- Batch numbers are contiguous from 0 through the current maximum.
...
2. Add `classificationBatch: 0` to all 18 catalog records.
3. Add `classification_batch: 0` to all 18 Markdown frontmatter blocks.
...
6. Update `content/ingestion-report.md` to identify the initial cohort as `Sınıflandırma 00` without rewriting its unrelated analysis.
...
- If legacy processed articles have no batch and the catalog contains only the initial cohort, migrate them to Batch 0.
...
3. Only then calculate `nextBatch = max(existing classificationBatch) + 1`; because the baseline is Batch 0, the first new cohort receives Batch 1.
...
Sınıflandırma 01 · 4 makale
  ...
```

The batch header is a compact archival folio marker integrated into the existing reading spine. It formats the numeric batch with `String(batch).padStart(2, "0")`, then combines that two-digit monospaced index with a plain Turkish label, article count, and a restrained rule. This is the one signature addition: the spine now records ingestion eras rather than being only a progress line.
...
- Schema parsing rejects missing, negative, fractional, or non-numeric batch values; zero is valid and reserved for the baseline cohort.
- Catalog validation rejects a missing baseline or gaps such as batches 0 and 2 without 1.
- Validation rejects batch interleaving in reading order, for example 0, 1, 0.
...
- Accept schema version 2 and valid non-negative batch values.
- Reject missing/invalid batch values.
- Reject a missing Batch 0, missing intermediate batch numbers, and interleaved blocks.
- Confirm all 18 real articles are Batch 0 after migration.
...
- Desktop sidebar shows `Sınıflandırma 00` and `18 makale`.
...
- All current articles and catalog records are visibly and structurally Batch 0.
- The first future successful import creates Batch 1, and every later successful import creates exactly one next batch shared by all new articles.
