...
tags:
  ...
  - hesap-butcesi
  - tekillestirme
content_hash: sha256:ed8e1ccfe53c30728e2e41f7f62da51c16d8fe74bc0eaf2185e760fa51f9e558
...
Common Crawl bir maden ocağıysa, C4 ve FineWeb gibi derlemler farklı zenginleştirme tesisleridir. Ama madende değerli olan şey nesnel biçimde ölçülebilir, oysa metinde "kalite"nin nesnel bir ölçüsü yoktur ve her ekip kendi kural setini kurar — C4 süslü parantez içeren her sayfayı kod diye atarken Dolma ve The Pile kodu kasten içerir. Tesisin gerçekte ne olduğu da buradan anlaşılır: filtreleme hattı bir dizi sezgisel kuraldan ve eğitilmiş sınıflandırıcıdan ibarettir, "iyi veri"nin tanımı ampiriktir ve ancak deneyle sınanır.
...
Tom Brown ve arkadaşlarının 2020 tarihli GPT-3 çalışması bunu açıkça gösteriyor. Havuz — yani eğitim için hazırlanan derlemin tamamı — beş kaynaktan oluşuyordu ve toplamı 499 milyar token'dı; eğitim bütçesi ise 300 milyar token'dı. Yani model havuzun tamamını bir kez bile okumadı. Buna karşılık bazı kaynakları defalarca okudu. İki kaynağı karşılaştır. Bütçenin yüzde 60'ı Common Crawl'dan geldi: 180 milyar token, oysa havuzda 410 milyar vardı — devasa ve gürültülü kaynağın yarısı bile okunmadı. Bütçenin yüzde 3'ü Wikipedia'dan geldi: 9 milyar token, oysa Wikipedia havuzunda yalnızca 3 milyar token vardı — yani Wikipedia kabaca üç kez baştan sona okundu. (Bu iki çarpma bizim; çalışmanın tablosu Wikipedia için 3,4 yazar, çünkü tablonun kendi ağırlıkları toplamda yüzde 101 ediyor ve satırlar birbirini tam tutmuyor. Büyüklük mertebesi değişmiyor.)

Verinin üzerinden bir tam geçişe epok (epoch) denir. Bu ölçekte bir koşu tam sayılı epoklarla ölçülmez: aynı koşuda Wikipedia üç kez okunurken Common Crawl'ın yarısı bile okunmamıştır. Hangi kaynaktan kaç kez okunacağı, veri karışımı (data mixture) denen ve elle verilen bir tasarım kararıdır; çalışmanın kendi metni bu ağırlıkların derlem boyutuyla orantılı olarak **kasten** belirlenmediğini, daha kaliteli sayılan kümelerin daha sık örneklendiğini yazar. Karışım, "hangi metne ne kadar güveniyoruz" sorusunun sayıya dökülmüş hâlidir.
...
Şimdi zinciri kuralım. 15 trilyon token, token başına 0,75 kelimeyle yaklaşık 11 trilyon kelime eder. Dakikada 238 kelimeyle bu yaklaşık 47 milyar dakika, yani 790 milyon saat okuma demektir. Hiç durmadan, uyumadan, günde yirmi dört saat okuyan bir insan için bu yaklaşık **doksan bin yıl** eder; günde sekiz saat okursa üç katı, yaklaşık 270 bin yıl. Varsayımları oynatmak sonucu değiştirir ama mertebesini değiştirmez: kelime/token oranını 0,65 alsak yaklaşık 78 bin, 0,80 alsak yaklaşık 96 bin yıl çıkar. Söylenebilecek cümle şu: on binlerce yıl. Aynı zincir 2020'nin GPT-3'ü için, 300 milyar token'la, yaklaşık bin sekiz yüz yıl verir. Dört yılda elli kat.

Yalnız model okumaz. İnsan okurken anlar, sorgular, önceki bilgisiyle çelişkiye düşer, kenara not alır, unutur ve hatırlar; model her token'da tek bir şey yapar, sıradakini tahmin eder ve hatası kadar ayarlanır. 15 trilyon token, 15 trilyon kez uygulanmış bir tahmin-ve-düzelt işlemidir; dakikada 238 kelime burada yalnızca bu büyüklüğü hissettirmek için kullanılan bir cetveldir.
...
![Öğrenme oranı eğrisi: işlenen token arttıkça oran önce kısa bir doğrusal ısınmayla tepe değerine çıkar, ardından uzun bir kosinüs sönümüyle tepe değerinin yüzde onuna iner ve orada düz kalır; yatay eksen ölçekli değildir, ısınma okunabilsin diye büyütülmüştür.](assets/ogrenme-orani-cizelgesi.svg "Şekil 3 — Isınma ve kosinüs sönümü")

Şekil 3'teki eğrinin asimetrisi meselenin kendisi: ısınma çok kısa, sönüm çok uzundur. Şekilde ısınma okunabilsin diye büyütüldü; gerçek oranda ısınmanın 375 milyon token'ı, 260 milyar token'lık sönümün yalnızca yaklaşık 1/700'üdür.

Bir de işin bölüşülmesi var. Llama 3, on altı bin grafik işlemcisine kadar çıkan bir kümede eğitildi ve iş dört ayrı eksende bölündü: veri, ağırlık matrisleri, katmanlar ve dizinin kendisi. Fabrika benzetmesiyle: veriyi bölmek, aynı montaj hattının binlerce kopyasını kurup her birine farklı kutular vermek ve vardiya sonunda öğrenilenleri ortaklaştırmaktır; modeli bölmek ise hattın kendisinin tek binaya sığmayıp parçalarının ayrı binalara dağıtılmasıdır. Fabrika benzetmesi burada yetersiz kalıyor: fabrikada binalar arası taşıma yavaşlatıcı bir ayrıntıdır, burada baskın maliyet kaleminin kendisidir — her adımda herkes herkesi beklemek zorundadır ve veri merkezinin elektrik çekişi onlarca megavat mertebesinde anlık dalgalanır. Teknik karşılığı iki ayrı bölme biçimidir: veri paralelliğinde (data parallelism) her işlemci aynı ağırlıkların bir kopyasını tutup farklı yığın parçası üzerinde gradyan hesaplar ve gradyanlar toplanır; model paralelliğinde (model parallelism) tek bir ağırlık matrisi ya da katman yığını işlemcilere bölünür.

Bu bölüşmenin bir bedeli var: Llama 3 ekibinin kendi ölçümüne göre donanımın teorik kapasitesinin ancak yüzde 38 ile 43'ü fiilen kullanılabildi; oran, kümenin büyüklüğüne ve bölme biçimine göre bu aralıkta oynuyor.
...
Bir mit düzeltmesi de tam buraya düşüyor. Popüler anlatı "GPT-3, 45 TB veriyle eğitildi" der. Birincil kaynağa bakalım: 45 TB, filtrelemeden **önceki** sıkıştırılmış düz metnin miktarıdır; filtrelemeden sonra 570 GB kalmıştır ve model bunun bile tamamını görmemiştir. Doğru cümle şu: model 300 milyar token işledi ve interneti okumadı.
...
Bu bölümdeki arıza ve yeniden başlatma sayılarının kaynağını da bilmelisin: hepsi şirketlerin kendi teknik raporlarından ve eğitim günlüklerinden geliyor ve hiçbiri hakemli yayın değil; bölümü kapatan GPT-3 itirafı ise hakemli bir yayından geliyor. Bu makalenin en çok sayı çektiği kaynak olan Llama 3 raporu da bir ön baskıdır. Yine de bunlar birincil kayıtlardır; bu ölçekte eğitim yapan ekiplerin sayısı azdır ve anlatan başka kimse yoktur.
