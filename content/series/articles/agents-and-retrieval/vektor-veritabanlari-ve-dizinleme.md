...
revision_note: "Ürün kuantizasyonu neden vektörü böldüğüyle sezgiden kuruldu; örnekteki hata açıklaması ve şekli düzeltildi, hoşgörülü bulma oranı tanımlandı."
---
## Sormadığımız soru
...
Baştan söyleyelim: bu makalede "dizin" sözcüğü, 42\. makaledeki ters dizinin vektör dünyasındaki karşılığı için kullanılıyor. Ters dizin her terim için belge listesi tutuyordu; **vektör dizini** (vector index) ise sorgu geldiğinde yirmi bir milyon vektörün yalnızca küçük bir kısmına dokunmayı sağlayan bir yapıdır. Yapıyı kurma işine de dizinleme diyeceğiz. 39\. makalede belleğe yazma adımı için de aynı İngilizce sözcük (indexing) geçiyordu; burada anlamı, vektörleri bir arama yapısına yerleştirmek.

## Tam taramanın faturası

Önce dizinsiz hâli hesaplayalım, çünkü dizinin varlık sebebi bu hesap.

Bir sorgu vektörü geldi. En yakın `k` belgeyi bulmanın en doğrudan yolu, sorguyu bütün belge vektörleriyle karşılaştırmaktır: 21.015.324 belge, her biri için 768 boyutlu bir nokta çarpım. Bu, sorgu başına 21.015.324 × 768 ≈ **16,1 milyar** çarpma-toplama demek. Alandaki adıyla **tam tarama** (brute-force search).
...
Roger Weber, Hans-Jörg Schek ve Stephen Blott'un 1998'de VLDB'de sunduğu çalışma bu fikrin yüksek boyutta neden çöktüğünü hem ölçtü hem kanıtladı. Ölçümde, veri ya da uzay bölmeye dayanan yapıların tamamı boyut sayısı **10 civarını** aştığında basit bir sıralı taramanın gerisine düşüyor. Kanıtta, her bölmeleme ve kümeleme yönteminin boyut belirli bir eşiği geçince ortalamada bütün bloklara dokunmak, yani taramaya dönüşmek zorunda kaldığı gösteriliyor; yazarların maliyet modelinde bu eşik 610 boyut, ama bu kaba bir üst sınır ve pratikte eşik çok daha aşağıda.
...
**İkincisi: sıkıştır.** Ters dosya hesabı azaltıyor ama 65 gigabaytı küçültmüyor; her vektör hâlâ 768 tane 32 bitlik sayı. İlk akla gelen sıkıştırma, birinci ailenin aracını yeniden kullanmaktır: k-ortalamalarla bir **kod defteri** (codebook) kur, her vektörü en yakın merkezin numarasıyla sakla. Ama numara 64 bayt, yani 512 bit tutacaksa defterde 2⁵¹² merkez gerekir; böyle bir defter ne öğrenilebilir ne saklanabilir. Tek büyük defterle yetinirsen de vektörü ancak kaba bir merkeze yuvarlayabilirsin.

Jégou, Douze ve Cordelia Schmid'in 2011 tarihli çalışmasının getirdiği **ürün kuantizasyonu** (product quantization) bu engeli vektörü bölerek aşıyor; Johnson ve arkadaşlarının çalışması mekanizmayı ayrıntısıyla yazıyor. Vektör `b` alt vektöre bölünür; her alt vektör kendi kod defterindeki 256 merkezden en yakınına yuvarlanır ve yalnızca o merkezin numarası saklanır. 256 seçenek bir bayta sığdığı için vektör `b` bayta iner. 768 boyutlu vektörü 64 alt vektöre bölersen 3.072 bayt yerine 64 bayt saklarsın: 48 kat küçük bir dizin, yirmi bir milyon vektör için 65 gigabayt yerine 1,3 gigabayt. Saklanan merkezler yalnızca 64 × 256 tanedir, ama her defterden birer seçim yapılabildiği için birlikte 256⁶⁴ = 2⁵¹² farklı yeniden kurulumu temsil ederler. Tek defterin imkânsız boyu, küçük defterlerin çarpımıyla elde ediliyor; "ürün" sözcüğü bu çarpımdan geliyor.

27\. makaledeki kuantizasyonla akrabalığı görüyorsun: orada ağırlıklar bir ızgaraya yuvarlanıyordu ve ızgaranın ölçeği bir sabitle veriliyordu; burada alt vektörler bir kod defterine yuvarlanıyor ve defterin kendisi veriden k-ortalamalarla öğreniliyor. Fark, yuvarlamanın tek tek sayılara değil küçük vektör parçalarına uygulanması.
...
Şimdi (0,30, 0,70, 0,70, 0,30) sorgusu gelsin. İlk yarısı (0,30, 0,70) için birinci defterin dört merkezine karesel uzaklıklar 0,02, 0,50, 0,08 ve 0,40; ikinci yarısı (0,70, 0,30) için 0,08, 0,72, 0,08 ve 0,26. Belgenin kodu (1, 1) olduğuna göre tahmini uzaklık iki tablonun ilk hücrelerinin toplamı: 0,02 + 0,08 = **0,10**. Gerçek karesel uzaklık ise 0,1225. Aradaki fark iki yuvarlamanın izi: ilk yarıda 0,95'in 0,8'e yuvarlanması o yarının uzaklığını 0,0725 yerine 0,02 gösteriyor, ikinci yarıda 0,80'in 0,9'a yuvarlanması 0,05 yerine 0,08. İki hata ters yönde olduğu için kısmen birbirini götürüyor ve toplamda 0,0225'lik fark kalıyor; tahmin, belgeyi olduğundan biraz yakın gösteriyor. Tablo bir kez kuruluyor; yirmi bir milyon belgenin her biri için yapılan iş iki tablo okuması ve bir toplama.

![Üç satırlı bir şema. Üst satırda belge vektörünün dört sayısı — 0,20, 0,95, 0,80, 0,10 — kutular hâlinde durur ve bir çizgiyle kod: (1, 1) · 4 bit kutusuna bağlanır; yanında her yarının en yakın merkeze yuvarlandığı ve yalnızca merkez numarasının saklandığı yazılıdır. Orta satırda sorgu vektörü 0,30, 0,70, 0,70, 0,30 ve yanında 1'den 4'e merkez numaralarıyla başlıklandırılmış iki uzaklık tablosu vardır: tablo 1'de 0,02, 0,50, 0,08, 0,40; tablo 2'de 0,08, 0,72, 0,08, 0,26. Kodun gösterdiği 1 numaralı hücreler vurgulanmıştır. Alt satırda tahmin: 0,02 + 0,08 = 0,10 kutusu, yanında gerçek karesel uzaklığın 0,1225 olduğu ve farkın iki yuvarlamadan — 0,95'ten 0,8'e ve 0,80'den 0,9'a — geldiği yazılıdır. En altta tabloların sorgu başına bir kez kurulduğu, belge başına yalnızca iki okuma ve bir toplama yapıldığı belirtilir.](assets/urun-kuantizasyonu.svg "Şekil 1 — Sıkıştırılmış belgeyle uzaklık: tablo oku, topla")
...
Ama 0,376 sayısını unutma: sıkıştırmanın bedeli doğruluktan ödeniyor. Bir milyar ölçeğinde belleğe sığan sıkıştırılmış dizinlerin gerçek en yakın komşuyu birinci sırada bulma oranı yüzde 50 civarında kalıyor, ve bu açık bir sonraki ailenin varlık gerekçesi.
...
Önce ölçünün tanımı, çünkü önemli bir ayrım buradan çıkacak. Çerçevede bulma oranı, dizinin döndürdüğü `k` sonuç içinde **tam taramanın bulacağı** `k` gerçek en yakın komşudan kaçının bulunduğudur. Dikkat: burada ilgililik etiketi yok. Ölçüt, sorguya gerçekten en yakın vektörlerle örtüşme; 29 ve 42\. makalelerdeki bulma oranı ise doğru **belgenin** bulunup bulunmadığına bakıyordu. Bu ikisini ayırt etmek için buradakine **dizin bulma oranı** diyeceğiz.

Her dizin için parametrelerin bütün kombinasyonları çalıştırılır, her koşu bir dizin bulma oranı ve saniyedeki sorgu sayısı verir ve yalnızca en iyi noktalar — belirli bir bulma oranında en hızlı olanlar — bir eğri olarak çizilir. Düğmeyi çevirmek dizini eğri üzerinde kaydırır: daha yavaş ama daha doğru bir noktaya ya da tersine.
...
Bunun bir bütçe sonucu var. Dizin, ilgili komşuların bulma oranı hedeflenerek ayarlandığında aynı kaliteyi yüzde 14 daha az hesapla veriyor. İlgililik etiketi olmayan kümeler için yazarlar bir vekil ölçü de öneriyor: **hoşgörülü bulma oranı** (tolerant recall), dönen bir sonucu, uzaklığı gerçek komşununkine belirli bir tolerans içinde yakınsa isabet sayar; neredeyse eşit uzaklıktaki, birbirinin yerine geçebilen komşuları ayırt etmeye çalışmaz. Hedef yüzde 95'lik geleneksel bulma oranından yüzde 95'lik hoşgörülü bulma oranına çevrildiğinde maliyet bir milyar vektörlük kümede yaklaşık dörtte bir, çok dilli bir Wikipedia getirme kümesinde yaklaşık üçte bir düşüyor; küçük bir kelime vektörü kümesinde kazanç yüzde 5'te kalıyor. Çünkü eğrinin sağ ucu diktir: son birkaç puanlık bulma oranı, en ilgisiz komşuları toplamak için harcanan en pahalı hesaptır.
...
Siddharth Gollapudi ve arkadaşlarının 2023 Web Konferansı'nda sunduğu çalışma bunun neden zor olduğunu ölçüyor. Etiketin **seçiciliğini** (specificity), dizindeki vektörlerin o etiketi taşıyan oranı olarak tanımlıyorlar. İki bariz yol var ve ikisi de kırılıyor. Sonradan süzme — dizini normal sorgula, etikete uymayanları at — seçicilik düşükse çöker: etiket vektörlerin binde birindeyse on sonuç için ortalama on bin komşu getirmek, yani sorgu genişliğini on binlere çıkarmak gerekir. Önceden süzme — her etiket için ayrı dizin — etiket sayısı arttıkça dizin sayısını patlatır. Çalışmanın önerisi, çizgeyi kurarken bağlantıları yalnızca geometriye göre değil etiket kümesine göre de seçmek; bu yolla seçiciliği onda birden milyonda bire kadar inen filtrelerde yüzde 90'ın üstünde bulma oranı korunuyor, öteki yaklaşımlar ise düşük seçicilikte anlamlı bir doğruluğa ulaşamıyor ve saniyede neredeyse bin kat daha az sorgu cevaplıyor.
...
Son bir düğüm 42\. makaleye bağlanıyor. Orada geç etkileşimli düzenin belgeyi tek vektör yerine token başına bir vektörle sakladığını ve bunun depolamayı artırdığını söylemiştik. Keshav Santhanam ve arkadaşlarının NAACL 2022'de sunduğu çalışma bu faturayı bu makalenin araçlarıyla ödüyor: token vektörlerini k-ortalamalarla bulunan merkezlere atayıp yalnızca merkezden kalan farkı birkaç bitle saklıyor ve depolamayı, kaliteyi koruyarak 6–10 kat düşürüyor. Aynı ekibin CIKM 2022'deki devamı, düşük puanlı belgeleri tam vektörlere hiç bakmadan eleyerek gecikmeyi kartta 7, işlemcide 45 kata kadar azaltıyor. Merkeze ata, farkı sıkıştır, önce kaba puanla ele: aynı üç fikir, bu kez belge başına yüzlerce vektör için.
