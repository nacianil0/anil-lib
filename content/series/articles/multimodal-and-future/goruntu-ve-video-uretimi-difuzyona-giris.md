...
tags:
  ...
  - kilavuzluk
  - ornekleyici
content_hash: sha256:45f92766a2c6a4b07f906020a71db02218f30b4882d079c865481bcf234c0a90
classification_version: 1
classification_batch: 20
revised_at: "2026-09-25"
revision_note: "İleri yönün formülü ve gürültüden temiz resme giden ters dönüşüm tek piksellik bir örnekle kuruldu; kılavuzluk katsayısının anlamı formülle açıklandı."
---
## Ters yöndeki soru
...
Asıl mesele ileri yönün **öğrenilmiyor** olması. Gürültü çizelgesini — hangi adımda ne kadar gürültü ekleneceğini — biz seçiyoruz, dolayısıyla ileri yönde bilinmeyen hiçbir şey yok. Üstelik ileri yön kapalı biçimde yazılabiliyor: sıfırıncı adımdaki temiz görüntüden `t`'inci adımdaki bozuk görüntüye, aradaki adımları tek tek atmadan tek hamlede gidilir. Sözle: bozuk görüntü, temiz görüntünün küçültülmüş bir kopyası ile ölçeklenmiş bir gürültünün toplamıdır. Sembolle, her piksel için:

`x_t = a · x_0 + b · ε`

Burada `x_0` temiz pikselin değeri, `ε` ortalaması sıfır ve varyansı bir olan Gauss gürültüsünden çekilmiş bir sayı, `a` ile `b` ise çizelgeden hesaplanan iki katsayıdır; ikisinin kareleri toplamı birdir (`a² + b² = 1`), böylece resim ölçeğini korurken sinyalden gürültüye doğru kayar. `a²`'ye **sinyal payı** diyelim: o adımda resmin ne kadarının hâlâ temiz görüntüden geldiğinin ölçüsü. Adımlar ilerledikçe sinyal payı 1'den 0'a iner. Kaynakta `a` katsayısı `√ᾱ_t` diye yazılır; okurken aynı nesne olduğunu bilmek yeter. Sinyal payının 0,25'e düştüğü adımda `a = 0,5`, `b = √0,75 ≈ 0,87`'dir; yani o adımdaki resim, temiz resmin yarısı ile gürültünün epeyce fazlasının toplamıdır.
...
Şekil 1'in alt satırındaki son not, düzenin neden bu kadar sade durabildiğini açıklıyor. Tek bir ağ, bin farklı zorluk derecesindeki aynı soruyu cevaplıyor: "bu resme ne kadar gürültü karışmış ve hangisi?" Gürültü düzeyi ağa girdi olarak verildiği için ağ hangi zorlukta çalıştığını biliyor.
...
### Gürültüyü bilen, resmi de bilir

O ortalama nereden geliyor, ve gürültüyü tahmin etmek neden yeterli bir hedef? Yukarıdaki denkleme dön: `x_t` elimizde, `a` ile `b` çizelgeden biliniyor; bilinmeyen yalnızca `x_0` ile `ε`. Birini bilen ötekini bulur: `x_0 = (x_t − b · ε) / a`. Gürültüyü doğru tahmin eden bir ağ, o adımda temiz resmi de kestirmiş olur; geri adımın ortalaması bu kestirimden hesaplanır.

Tek bir piksel üzerinde izleyelim; sayılar açıklama amaçlı, kendi seçimimiz. Temiz pikselin değeri `x_0 = 0,8`, çekilen gürültü `ε = −0,4`, sinyal payı 0,25 (yani `a = 0,5`, `b ≈ 0,87`). Bozuk piksel: `0,5 × 0,8 + 0,87 × (−0,4) = 0,40 − 0,35 = 0,05`. Ağ yalnızca bu 0,05'i ve adım numarasını görüyor. Gürültüyü tam bilseydi `(0,05 + 0,87 × 0,4) / 0,5 ≈ 0,80` ile temiz değeri geri bulurdu. Biraz yanılıp `−0,3` derse kestirimi `(0,05 + 0,26) / 0,5 ≈ 0,62` olur: yönü doğru, ayrıntısı eksik. Çok gürültülü bir adımda temiz resmin kestirimi bu yüzden bulanık bir taslaktır.

Örnekleyicinin işi bu taslağı kullanmaktır. Her adımda, kestirilen temiz resim ile şimdiki gürültülü resim arasından, bir sonraki — biraz daha az gürültülü — düzeye uyan bir ara resim kurar; özgün örnekleyici buna taze bir miktar gürültü de ekler. Sinyal payı arttıkça ağın işi kolaylaşır ve taslak keskinleşir. Bu okumanın bir sonucu birazdan önemli olacak: her adımda temiz resmin bir kestirimi elde olduğuna göre, örnekleyici gürültü düzeylerini tek tek yürümek zorunda değildir.
...
Omer Bar-Tal ve arkadaşlarının SIGGRAPH Asia 2024'te sunduğu çalışma yaygın bir tasarım kararını eleştiriyor. Video modellerinin çoğu önce birbirinden uzak anahtar kareler üretip aralarını zamansal bir üst-örnekleyiciyle dolduruyor; yazarlara göre bu düzen küresel zamansal tutarlılığı doğası gereği zorlaştırıyor, çünkü hızlı hareket seyrek örneklenmiş karelerde kayboluyor. Önerdikleri mimari videonun **bütün zamansal süresini tek geçişte** üretiyor ve bunu hem uzamsal hem zamansal olarak çoklu ölçekte çalışarak yapıyor.
...
**Adım sayısı modelin değil örnekleyicinin özelliğidir.** Aynı eğitilmiş ağda, elli adımda örnekleyicinin seçimi FID'i 1,7 ile yedi kat arasında değiştiriyor; damıtma bu sayıyı dört adıma, tek adıma kadar indirebiliyor.
...
- Bar-Tal, O., Chefer, H., Tov, O., Herrmann, C., Paiss, R., Zada, S., Ephrat, A., Hur, J., Liu, G., Raj, A., Li, Y., Rubinstein, M., Michaeli, T., Wang, O., Sun, D., Dekel, T. & Mosseri, I. (2024). *Lumiere: A Space-Time Diffusion Model for Video Generation*. SIGGRAPH Asia 2024 Conference Papers, s. 1–11. [Bağlantı](https://doi.org/10.1145/3680528.3687614)
