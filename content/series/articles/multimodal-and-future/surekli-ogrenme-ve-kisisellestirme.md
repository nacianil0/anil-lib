...
tags:
  ...
  - felaket-unutmasi
  - getirme
content_hash: sha256:cf3079bba755a7a1ab95a135b108494f49e0e116b1f8835c2295ebe07cb5f597
...
**Cerrahi düzenleme.** Alandaki adıyla **model düzenleme** (model editing): tek bir olguyu, tek bir küçük güncellemeyle değiştirmek ve geri kalan her şeye dokunmamak. Bu makalenin asıl konusu, çünkü en çekici seçenek bu ve faturası en az görünen de bu.
...
"Rank-bir" sözcüğü 92\. makaleden tanıdık ve burada tam olarak ne yaptığını iki satırda görmek mümkün. Sayılar bizim; gerçek yöntem araya bir de eşdeğişirlik terimi koyuyor, mekanizmayı görmek için onu bir kenara bırakıyoruz. İki boyutlu bir bellek düşün: `W` birim matris olsun, yani her anahtar kendi değerine gitsin. Düzenlemek istediğimiz olgunun anahtarı `k* = (1; 0)` ve mevcut değeri `W k* = (1; 0)`. İstediğimiz yeni değer `v* = (0; 1)`. Güncelleme, istenen ile mevcut arasındaki farkın anahtarla dış çarpımıdır: `Δ = (v* − W k*) k*ᵀ`, yani `(−1; 1)` sütunuyla `(1; 0)` satırının çarpımı — ilk sütunu `(−1; 1)`, ikinci sütunu sıfır olan bir matris. Toplayınca `W′` ilk sütunu `(0; 1)`, ikinci sütunu `(0; 1)` olan matrise dönüşüyor.

Şimdi iki sınavı yapalım. `W′ k* = (0; 1)`: düzenleme tuttu. Şimdi anahtara **dik** bir başka olgunun anahtarını deneyelim, `k′ = (0; 1)`: `W k′ = (0; 1)` idi, `W′ k′` de `(0; 1)`. Hiç değişmedi.

Bu iki satır yöntemin hem gücünü hem sınırını aynı anda söylüyor. Güncelleme yalnızca `k*` yönünde etki eder; o yöne dik duran her şey korunur. "İlgisiz bilgilere dokunmama" özelliği bir başarı değil, bir kimliktir — matematikten çıkar. Ama aynı kimlik, olgunun **sonuçlarının** da dokunulmadan kaldığı anlamına gelir: "şu kişinin eşi" ile "şu kişinin kayınbiraderi" farklı anahtarlardır, ve ikincisi birincisine dik olduğu ölçüde güncellemeden hiç etkilenmez.
...
**Düzenlemelerin birikimi kademeli değil, kademeli sonra ani.** Tutma oranı 100 ile 1.000 düzenleme arasında bir yerde düşmeye başlıyor, ve tek bir devre dışı bırakan düzenleme modeli bitiriyor; o düzenlemenin ağırlıktaki izi ötekilerin üç mertebe üstünde ve ilk düzenleme olarak uygulansa da aynı sonucu veriyor.
