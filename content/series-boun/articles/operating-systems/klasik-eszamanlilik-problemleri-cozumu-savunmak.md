...
tags:
  ...
  - kilitlenme
content_hash: sha256:bab21f7e2a7d1ec8b2e971247dcc7263d3bd3caa29f67885b300f989dd8ccf04
...
Bu iki kurulumu da kaba kuvvetle taradım. Kilit en içteyken erişilebilir 10 durumun hiçbirinde iki iş parçacığı da bloke değil; kilit en dıştayken erişilebilir 14 durumun ikisi kilitlenmedir. Biri yukarıda anlattığım durumdur (tüketici `mutex`i tutuyor, `dolu`yu bekliyor; üretici `mutex`i bekliyor), öteki onun aynasıdır: tampon doluyken üretici `mutex`i tutup `bos`u bekler, tüketici `mutex`i bekler. Bu sayılar kaynaktan değil, kendi programımdandır; modelde bir üretici ve bir tüketici sonsuz döngüde çalışır, MAX = 1'dir ve durum iki program sayacı ile üç semaforun değeridir.
...
![İki panelli şema. Solda sınırlı tampon, MAX = 4 ve sayaç = 2: dört hücreli dizinin ilk iki hücresi vurgulu ve dolu, son ikisi boş; al indisi birinci, koy indisi üçüncü hücreyi gösterir ve kesik bir ok modülo ile sarmalar. Üretici sayaç == MAX iken, tüketici sayaç == 0 iken bekler; iki koşul değişkeni var, üretici boş'u bekleyip dolu'yu, tüketici dolu'yu bekleyip boş'u bildirir. Sağda iki hata, iki iz: tek hücreli tamponda tek koşul değişkeni kullanılırsa bildirim öteki tüketiciye gider ve üçü de uykuda kalır; vurgulu kutuda semaforlarda yanlış sıra, tüketici kilidi tutarak dolu'yu bekler, üretici kilidi bekler, doğrusu kilit en içte. Kaba kuvvet taraması: kilit en dıştayken 14 durumun içinde kilitlenme var, en içteyken 10 durumun hiçbirinde yok](assets/sinirli-tampon.svg "Şekil 1 — Dört adımlı kuruluş: her adım bir hatayı kırar, sonuncusu sırayla ilgilidir")
...
![İki panelli şema; ikisinde de düğümleri saat yönünde P0'dan P4'e dizili birer beşgen graf var. Solda naif çözüm, herkes önce solundakini alır: vurgulu beş ok P0'dan P1'e, P1'den P2'ye, P2'den P3'e, P3'ten P4'e ve P4'ten P0'a gider ve döngü kapanır. Not: her filozof solunu tutar, sağını bekler; bu bir kilitlenmedir. Sağda asimetrik sıra, P4 önce sağındakini alır: ilk dört ok aynıdır, vurgulu beşinci ok ise P0'dan vurgulu P4'e gider; 0 numaralı çatalı P4 önce aldığı için o kenarın yönü değişir ve döngü kapanamaz. Alt satırlar: naif kurulumda 82 erişilebilir durumun içinde kilitlenme var, asimetrik kurulumda 70 durumun hiçbirinde yok; ikisinde de en çok iki filozof yiyebilir](assets/filozoflar-dongusu.svg "Şekil 2 — Kilitlenmenin çekirdeği bir döngüdür; tek bir oku çevirmek onu kapanamaz yapar")
