...
tags:
  ...
  - kilit-sirasi
  - livelock
content_hash: sha256:6f67350fb670e973b35babae15ca2873116e2da93f1c8d8d8fdd1f83c301657c
...
Bu iki kurulumun bütün erişilebilir durumlarını taradım. Ters sıralı kurulumda **19 erişilebilir durum** var ve içlerinden **yalnızca biri** kilitlenmedir; ikisi de aynı sırayla aldığında erişilebilir **16 durumun hiçbiri** kilitlenme değil. Bu sayılar kaynaktan değil, kendi programımdandır ve bir şeyi çok net söylüyorlar: kilitlenme kodun bir özelliği değil, **belirli bir izin** özelliğidir. Yüz koşuda görünmeyip yüz birincide görünmesi tam olarak bu yüzdendir.

Peki hangi koşullarda mümkün oluyor? Klasik cevap 1971 tarihli bir çalışmadan gelir ve dört maddedir:

- **Karşılıklı dışlama (mutual exclusion):** iş parçacıkları ihtiyaç duydukları kaynak üzerinde dışlayıcı denetim iddia eder — bir kilidi almak tam olarak budur.
- **Elde tutup bekleme (hold-and-wait):** iş parçacığı kendine ayrılmış kaynakları **tutmaya devam ederken** bir başkasını bekler.
...
![Dikey bir çizgiyle ayrılmış iki panelli bir şema. Sol panelin başlığı iki kilitli en küçük kilitlenme. Panelde dört dikdörtgen kutu bir kare oluşturacak biçimde yerleşmiş: sol üstte T1, sağ üstte L2, sol altta L1, sağ altta T2; L1 ve L2 kutuları vurgulanmış. Kutuları dört ok saat yönünde bir döngü hâlinde bağlıyor: T1'den L2'ye giden yatay okun üstünde bekliyor, L2'den T2'ye giden dikey okun sağında sahibi, T2'den L1'e giden yatay okun altında bekliyor, L1'den T1'e giden dikey okun sağında sahibi yazıyor. Panelin altında iki satır var: T1 L1'i tutup L2'yi, T2 L2'yi tutup L1'i ister; dört ok kapalı bir döngü oluşturur. Sağ panelin başlığı dört koşul ve her birini kaldırmanın yolu. Panelde dört numaralı madde alt alta duruyor ve her maddenin altında girintili bir kaldırma satırı var: bir, karşılıklı dışlama, kaldır kilitsiz veri yapısı CAS; iki, elde tutup bekleme, kaldır bütün kilitleri tek seferde al; üç, önkesmesizlik, kaldır deneme kilidiyle geri çekil; dört, döngüsel bekleme, kaldır küresel kilit sırası koy. Dördüncü madde ve kaldırma satırı vurgulanmış, altında pratikte en çok kullanılan dördüncüsüdür notu var. Şemanın en altında üç satır daha var: dördünden biri sağlanmazsa kilitlenme olamaz, dördü birden sağlansa bile olmak zorunda değildir; kaba kuvvetle taradım, ters sırada on dokuz erişilebilir durumun yalnızca biri kilitlenme; aynı sırada alındığında on altı durumun hiçbiri kilitlenme değil](assets/dort-kosul.svg "Şekil 1 — Dört koşul aynı anda sağlanmalı; her önleme tekniği bir koşulu hedefler")
...
Sıranın koda nasıl geçtiğini gösteren küçük ve zarif bir numara var. İki kilidi parametre olarak alan bir fonksiyon düşün; çağıranlardan biri ikisini bir sırayla, öteki ters sırayla verirse fonksiyon "hep önce birinciyi al" kuralını uygulasa bile kilitlenir. Çözüm, sırayı içerikten değil **kilidin adresinden** türetmektir:
...
Bunun için sistemin gelecek hakkında bir şey bilmesi gerekir. En basit bilgi, her sürecin **en fazla** ne kadar kaynak isteyeceğidir. Fikri ortaya atan metin, problemi 1965'te tam olarak bu varsayımla kurar: bellek sayfalarını "ödünç alan" süreçler ve her sürecin önceden bildirdiği bir üst sınır.
...
![Dikey bir çizgiyle ayrılmış iki panelli bir tablo şeması. Sol panelin başlığı Dijkstra'nın örneği, toplam sermaye yüz sayfa. Panelde dört sütunlu bir tablo var, sütun başlıkları süreç, ihtiyaç, borç ve kalan istek. Birinci satır P1 seksen kırk kırk, ikinci satır P2 altmış yirmi kırk. Tablonun altında yatay bir çizgi ve onun altında kasa eşittir yüz eksi altmış eşittir kırk yazıyor. Onun da altında vurgulanmış bir kutunun içinde GÜVENLİ yazıyor. Sağ panelin başlığı ikisine de birer sayfa daha verilirse. Aynı dört sütunlu tablo var: birinci satır P1 seksen kırk bir otuz dokuz, ikinci satır P2 altmış yirmi bir otuz dokuz. Altında kasa eşittir yüz eksi altmış iki eşittir otuz sekiz yazıyor ve onun altındaki soluk kutunun içinde GÜVENSİZ yazıyor. Şemanın alt kısmında altı satır var: güvenlik denetimi, kasa en az bir sürecin kalan isteğini karşılıyor mu, karşılıyorsa o süreç bitirilir ve borcunu geri verir, kasa büyür ve sonraki süreç denenir; hepsi bitebiliyorsa durum güvenlidir, burada kasa otuz sekiz ve iki kalan istek de otuz dokuz olduğu için hiçbiri bitemez; yalnızca birine verilseydi kasa otuz dokuz olurdu ve o süreç bitip borcunu geri verirdi, yani güvenli; güvensiz durum kilitlenmiş durum değildir, kilitlenmenin artık mümkün olduğu durumdur; sayılar Dijkstra'nın kendi örneğindendir ve güvenlik denetimi kendi programımla doğrulanmıştır](assets/bankaci-guvenli-durum.svg "Şekil 2 — Güvensiz durum kilitlenmiş durum değildir; kilitlenmenin mümkün olduğu durumdur")

Bu ayrımı mülakatta kaçırmamak gerekir: **güvensiz durum kilitlenmiş durum değildir.** Güvensiz bir durumda süreçler pekâlâ ilerleyip sorunsuz bitebilir; algoritma yalnızca en kötü ihtimalin artık kapatılamadığını söyler. Bankacı bu yüzden **muhafazakârdır** ve bedeli budur: hiç yaşanmayacak bir felaketi önlemek için gereksiz bekletme yapar.
...
Tespit, graf algoritmaları makalesindeki döngü tespitinin doğrudan uygulanmasıdır. Sistem **kaynak atama grafını (resource-allocation graph)** kurar — süreçlerden kaynaklara "istiyor" okları, kaynaklardan süreçlere "sahibi" okları — ve grafta döngü arar. Veritabanı sistemleri tam olarak bunu yapar: bir kilitlenme dedektörü belirli aralıklarla çalışır, döngü bulursa işlemlerden birini geri alıp yeniden başlatır. İşletim sistemi tarafında kurtarma çoğu zaman daha kabadır; yılda bir donan bir sistemi yeniden başlatmak, nadir bir olay için karmaşık bir düzenek yazmaktan ucuzdur. Bu, mühendislikte sık karşılaşılan bir karardır: bir kötülük nadirse ve bedeli küçükse, onu önlemeye harcanacak emek başka yerde daha çok kazandırır.
...
|---|---|---|---|
| Kilitlenme | Hayır | Hayır | Hayır (hepsi bloke) |
| Açlık | Evet | Hayır | — |
| Livelock | Hayır | Hayır | Evet (sürekli deneme) |
| Öncelik tersine dönmesi | Evet | Hayır (yüksek öncelikli bekler) | — |
