...
tags:
  ...
  - yaris-kosulu
  - kritik-kesim
content_hash: sha256:78268eef1b11b59dbeda90b12c15cd623d224d4a813dd80ae114ad6591d84672
...
Bu makine üzerinde korunan değişmez şudur: **her an en fazla bir süreç çalışan durumdadır** (tek işlemcide) ve engellenmiş bir süreç, beklediği olay gerçekleşmeden hazır kümesine geri konmaz. İkinci koşul, kilitlenme makalesinde göreceğimiz sorunun da kapısıdır: beklenen olay hiç gerçekleşmezse süreç engellenmiş durumda sonsuza kadar kalır.
...
![İki panelli şema. Solda süreç durum makinesi: vurgulu ÇALIŞAN, HAZIR ve ENGELLENMİŞ kutuları. Çalışandan hazıra çizelgeden çıkar, hazırdan çalışana çizelgele okları; çalışandan engellenmişe giriş/çıkış başlat, engellenmişten hazıra giriş/çıkış bitti okları var. Kesik çizgili iki soluk kutu daha var: başlangıçtan hazıra ve çalışandan zombiye soluk oklar gider. Notlar: tek işlemcide her an en fazla bir süreç ÇALIŞAN durumdadır; engellenmiş süreç, olayı gelmeden hazır olmaz. Sağda süreç denetim bloğu, PCB: süreç kimliği, durum, ebeveyn işaretçisi, bellek başlangıcı ve boyutu, çekirdek yığınının tabanı; vurgulu yazmaç bağlamı kutusunda program sayacı, yığın işaretçisi ve genel amaçlı yazmaçlar; ardından açık dosyalar, çalışma dizini, tuzak çerçevesi. Notlar: bütün kayıtlar süreç listesi, yani görev listesi; durdurmak kaydetmek, sürdürmek geri yüklemektir](assets/surec-durum-makinesi.svg "Şekil 1 — Sürecin üç durumu ve çekirdeğin her süreç için tuttuğu kayıt")
...
Tek bir süreç hakkındaki bilgiyi tutan yapıya **süreç denetim bloğu (Process Control Block, PCB)** ya da süreç tanımlayıcısı denir. Şekil 1'in sağ paneli, xv6 öğretim çekirdeğinin gerçekten tuttuğu alanların çoğunu gösteriyor; tam liste şöyle: süreç belleğinin başlangıcı ve boyutu, çekirdek yığınının tabanı, süreç durumu, süreç kimliği, ebeveyn işaretçisi, uyunan kanal, öldürülme bayrağı, açık dosyalar, çalışma dizini, **yazmaç bağlamı** ve o anki kesme için tuzak çerçevesi. Linux, macOS ya da Windows'un karşılıkları çok daha karmaşıktır ama iskelet aynıdır.
...
Bu sayıları çevrime çevirmek öğretici bir sonuç veriyor ve aritmetiği kendim yaptım: 200 MHz'de 6 mikrosaniye **1.200 çevrim** demektir. Kaynak modern sistemlerin neredeyse on kat daha iyi olduğunu ve mikrosaniyenin altına indiğini söylüyor; 6 mikrosaniyenin onda biri olan 0,6 mikrosaniye 2 GHz'de **1.200**, 3 GHz'de **1.800 çevrim** eder. Yani geçen otuz yılda süre yaklaşık on kat düşerken **çevrim sayısı düşmedi**: kaynağın ifadesiyle bu sonuçlar işlemci başarımını kabaca izledi, süre saat hızıyla birlikte kısaldı. Kaynak buna bir uyarı da ekliyor: Ousterhout'un gözlemine göre işletim sistemi işlemlerinin çoğu bellek yoğundur ve bellek bant genişliği işlemci hızı kadar iyileşmediği için her işlem bu hızlanmayı izlemez. Bellek hiyerarşisi makalesinde bu gözlem donanım tarafından tekrar karşımıza çıkacak.
...
İkinci fark yığındadır. Klasik bir sürecin adres uzayında tek bir yığın vardır ve genellikle adres uzayının en yüksek adreslerinde durur — Şekil 2'de adresler yukarıdan aşağıya arttığı için şeridin en altında. Çok iş parçacıklı bir süreçte **her iş parçacığı için bir yığın** olmalıdır, çünkü her biri bağımsız çalışır ve kendi fonksiyon çağrılarını yapar. Yığında tutulan yerel değişkenler, parametreler ve dönüş değerleri bu yüzden **iş parçacığına özel depolamada (thread-local storage)** durur. Terime dikkat: OSTEP bu adı ilgili iş parçacığının yığını için kullanır; C ve C++ pratiğinde ise *thread-local storage* daha çok `_Thread_local` gibi, her iş parçacığında ayrı bir kopyası olan global değişkenleri anlatır. Mülakatta hangisini kastettiğini söylemek yeter.
...
![İki panelli şema; ikisinde de adresleri yukarıdan aşağı artan dikey bir bellek şeridi var. Solda tek iş parçacıklı adres uzayı: 0 KB'de kod, 1 KB'de heap, 2 KB'den başlayan boş alan ve 15 ile 16 KB arasında vurgulu tek yığın; heap aşağı, yığın yukarı, boş alana doğru büyür. Not: tek yığın, en yüksek adreslerde. Sağda çok iş parçacıklı adres uzayı: kod ve heap aynı yerde, ama iki vurgulu yığın var, yığın 2 ortada, yığın 1 en altta. Yan notlar: kod ve heap paylaşılır, her iş parçacığı kendi yığınını tutar. Alt notlar: iş parçacığı geçişinde sayfa tablosu değişmez; her iş parçacığı için ayrı bir TCB tutulur](assets/surec-ve-is-parcacigi-bellek.svg "Şekil 2 — Paylaşılan ve paylaşılmayan: kod ile heap ortak, yığın iş parçacığına özel")

Şekil 2'deki fark, iş parçacığının neden kullanıldığını da açıklıyor. **İki gerekçe** vardır.

**Birincisi paralellik.** Çok büyük diziler üzerinde işlem yapan bir programı tek işlemcide çalıştırmak doğrudandır; birden çok işlemci varsa işi bölerek hızlandırabilirsin. Bunun adı paralelleştirmedir ve işlemci başına bir iş parçacığı doğal yoldur. Alt sınırlar makalesinde kurduğumuz **iş ile açıklık** ayrımı buranın analiz aracıdır: iş toplam komut sayısıdır, açıklık bağımlılık zincirinin uzunluğudur ve hızlanmanın tavanını açıklık belirler. Modelin iş yasası gereği açıklık ne kadar kısa olursa olsun P işlemcide hızlanma P'yi geçemez; işletim sistemi tarafında buna bir kısıt daha eklenir: iş parçacığı yaratmanın ve aralarında geçişin maliyeti bu tavanı aşağı çeker.

**İkincisi yavaş giriş/çıkış nedeniyle tıkanmamak.** Bir iş parçacığı diski ya da ağı beklerken engellenirse, çizelgeleyici aynı sürecin çalışmaya hazır başka bir iş parçacığına geçebilir. Bu, çoklu programlamanın süreçler için yaptığı şeyin **tek bir programın içinde** yapılmış hâlidir; web sunucuları ve veritabanı yönetim sistemleri bu yüzden iş parçacığı kullanır.
...
Sebep, tek satırlık artırmanın tek bir komut olmamasıdır. Derleyici bunu üç komuta çevirir: değeri bellekten bir yazmaca **yükle**, yazmaca bir **ekle**, yazmacı belleğe **geri yaz**. Toplamda 60 milyon komut yürütülür. Tek tek her komut bölünmezdir; bölünebilen, üç komutluk dizinin kendisidir — zamanlayıcı kesmesi herhangi iki komutun arasına girebilir.
