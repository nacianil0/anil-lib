...
tags:
  ...
  - medyan-bulma
  - ozyineleme-agaci
content_hash: sha256:129f00ebbc233a87b4d69ed47ed05301111b7ac2f184fe7364ea54ee46dc036a
...
![Kesikli bir dikey çizgiyle ayrılmış iki özyineleme ağacı. Solda naif bölme, 4 alt problem, T(n) = 4T(n/2) + Θ(n): kök n, altında dört n/2 kutusu ve 16 düğüm, her biri n/4. Altında derinlik = log₂ n, yaprak = 4^(log₂ n) = n² ve sonuç Θ(n²), okul yöntemiyle aynı sınıf. Sağda Karatsuba, 3 alt problem, T(n) = 3T(n/2) + Θ(n): kök n, üç n/2 kutusu ve 9 düğüm, her biri n/4. Derinlik yine log₂ n, yaprak = 3^(log₂ n) = n^(log₂ 3), sonuç Θ(n^1,585). Alttaki not: değişen tek şey alt problem sayısıdır, küçülme çarpanı b ve birleştirme maliyeti g(n) aynı kaldı; n = 1024 basamakta yaprak sayısı 1.048.576'ya karşı 59.049, yaklaşık 18 kat](assets/karatsuba-dallanma-carpani.svg "Şekil 1 — Alt problem sayısını dörtten üçe indirmek: aynı ağaç, farklı sınıf")
...
İki alt problem var ve boyutları farklı; tek bir a ve b çifti yazılamaz. Çözüm için tahmin-et-ve-tümevarımla-doğrula yöntemine dönülür. Sezgi, alt problem paylarının toplamının 1/5 + 7/10 = 9/10 olması, yani girdinin tamamından küçük kalmasıdır; bu, işin seviyeler boyunca geometrik olarak azalacağını düşündürür. T(n) ≤ c·n tahmin edilir ve yeterince büyük bir c için tümevarım yürür. Tavanı ve +6'yı bir kenara bırakıp Θ(n) terimini bir s·n ile sınırlarsak (burada s yalnızca bir sabittir, alt problem sayısı a ile karıştırma) c·n/5 + 7c·n/10 + s·n = 9c·n/10 + s·n ≤ c·n eşitsizliği c ≥ 10s seçildiğinde sağlanır. Tavan ve +6 hesaba katılınca sol taraf en fazla 9c·n/10 + 7c + s·n olur ve aynı eşitsizlik n ≥ 140 için c ≥ 20s ister; sonuç değişmez, yalnızca sabit büyür. Sonuç **Θ(n)** — sıralamadan bile ucuz, doğrusal zamanda medyan.
...
Grup boyutunun neden beş olduğu ayrı bir tasarım dersidir. Üçerli gruplarla yapsaydın garanti edilen eleme oranı 1/3'e düşer ve bağıntı T(n) = T(n/3) + T(2n/3) + Θ(n) olurdu; payların toplamı **tam olarak 1** eder ve geometrik azalma kaybolur. O bağıntının çözümü Θ(n log n)'dir, yani baştaki sıralama fikrine geri dönmüş olursun. Sayıyla da görülür — aşağıdaki oranları iki bağıntıyı kendi kodumla çözerek hesapladım; Θ(n) terimini tam n aldım, n'yi gerçel sayı gibi böldüm ve taban olarak n < 2 iken T(n) = n koydum: üçerli grupla T(n)/n oranı 1000, 10⁴, 10⁵, 10⁶ ve 10⁷ için 11,4 → 15,0 → 18,7 → 22,3 → 25,9 diye her onlukta yaklaşık 3,6 artar — logaritmik büyüme budur. Beşerli grupla aynı oranlar 6,9 → 7,9 → 8,5 → 9,0 → 9,3 diye artar ve 1/(1 − 0,9) = 10 sınırına yaklaşarak durur. Beş, bu eşitsizliği sağlayan en küçük tek sayıdır.
