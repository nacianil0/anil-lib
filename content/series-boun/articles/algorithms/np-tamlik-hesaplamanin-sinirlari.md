...
tags:
  ...
  - sozde-polinom
content_hash: sha256:a69fa9a39f91757d62939851a7f98f1d187954ca1d0bc1f4590e1ae4a7f55254
classification_version: 1
classification_batch: 8
revised_at: "2026-09-25"
revision_note: "Durma ispatı tutarlı hâle getirildi; 3SAT çevirisinin eş-sağlanabilir olduğu ve 2-SAT teoreminin iki yönü ayrı ayrı açıklandı."
---
...
Somut ve isimli bir örnek de var. **Durma problemi (halting problem):** verilen bir program verilen bir girdide duruyor mu? İspat iki satırdır ve mantık makalesindeki çelişkiyle ispat kalıbının en bilinen uygulamalarından biridir.

Diyelim ki durma problemini çözen bir `P` makinesi var: `P(M, x)`, `M` makinesi `x` girdisinde duruyorsa EVET, sonsuza kadar çalışıyorsa HAYIR der ve kendisi her zaman durur. `P`'yi küçük bir değişiklikle `Q` makinesine çevirelim: `Q(M)`, `P(M, M)` sorusunu sorar, yani `M`'ye kendi tanımı girdi olarak verilince ne olacağını; `M(M)` **duruyorsa sonsuza kadar çalışsın**, `M(M)` sonsuza kadar çalışıyorsa **dursun**. Şimdi tek soru: `Q(Q)` ne yapar? Duruyorsa, tanımı gereği sonsuza kadar çalışması gerekir; sonsuza kadar çalışıyorsa, tanımı gereği durması gerekir. Tek çıkış, `P`'nin en baştan var olmamasıdır.
...
Karar verilebilir problemler de kendi içinde katmanlıdır. **R** sonlu zamanda karar verilebilen problemler, **EXP** üstel zamanda (2 üzeri n^O(1)) karar verilebilenler, **P** ise polinom zamanda (n^O(1)) karar verilebilenlerdir. Bu üç sınıf birbirinden kesin olarak ayrıdır: P, EXP'in ve EXP, R'nin **öz** altkümesidir. n × n tahtaya genelleştirilmiş satranç, EXP'te olup P'de olmayan problemlerden biridir; olağan 8 × 8 satranç sabit boyutlu olduğu için bu sınıflandırmanın konusu değildir.
...
![İki panelli bir küme şeması. Sol panel P ≠ NP varsayımı altında: büyük NP elipsinin içinde, solda en kısa yol yazılı küçük P elipsi; kesik çizgili NP-zor dikdörtgeni elipsin sağ kısmını örtüp dışarı taşıyor, ikisinin kesişimi vurgulu NP-tam bölgesi. Altında örnekler: P için en kısa yol, 2-SAT, 2-boyama; NP-tam için 3SAT, altküme toplamı, 3-boyama; NP-zor olup NP'de olmayan durma problemi; NP-tam = NP ∩ NP-zor. Sağ panel P = NP olsaydı: NP-zor dikdörtgeninin içinde P = NP = NP-tam etiketli tek elips ve elipsin dışında durma problemi. Altında dört sonuç ve bir kayıt: her NP problemi polinom zamanda çözülür, zorluk varsayımlı kriptografinin temeli çöker, NP-zorun NP dışı kısmı değişmez, durma problemi yine çözülemez; iki önemsiz dil dışında her NP dili NP-tam olur. Alttaki not: NP-zor olmak NP'de olmayı gerektirmez](assets/p-np-kapsama.svg "Şekil 1 — İki panel: P ≠ NP varsayımı altında ve P = NP durumunda sınıfların yerleşimi")
...
İlk NP-tam problemi 1971'de Cook buldu: **SAT** ve onun üç literalli biçimi **3SAT** — verilen bir Boolean formülü doğru yapan bir değişken ataması var mı? NP-zorluğunun sezgisi şudur: NP'deki her problemin polinom zamanlı bir doğrulayıcısı vardır, her doğrulayıcı VE/VEYA/DEĞİL kapılarından oluşan polinom boyutlu bir devre olarak yazılabilir ve her devre polinom boyutlu bir 3SAT formülüne çevrilebilir. Son adımda bir incelik var. Boolean cebiri makalesindeki mantıksal eşdeğerlik kurallarıyla, örneğin dağılma kuralıyla, devreyi doğrudan eşdeğer bir formüle açmak formülü üstel büyütebilir. Polinom çeviri bunun yerine devrenin her teline yeni bir değişken atar ve her kapıyı yalnızca kendi giriş ve çıkış değişkenleri arasında küçük bir koşul olarak yazar. Ortaya çıkan formül devreyle eşdeğer değil **eş-sağlanabilirdir (equisatisfiable)**: devreyi 1 yapan bir girdi varsa formül sağlanabilir, yoksa sağlanamaz. İndirgemenin istediği de tam olarak budur.
...
![İki panel. Sol panel doğru yön, bilinen zordan yeniye: 3SAT kutusundan vurgulu X kutusuna polinom çeviri etiketli kalın ok, 3SAT ≤p X; X kolaysa 3SAT de kolaydır, 3SAT zorsa X de zordur; vurgulu kutu: bu yön X'in NP-zor olduğunu gösterir. Somut örnek 3SAT ≤p bağımsız küme: 4 clause ve 3 değişken 12 düğüm ve 24 kenara çevrilir, her clause bir üçgen, çelişkili literaller bağlı; formül sağlanabilir ⟺ boyutu 4 olan bağımsız küme var. Sağ panel yanlış yön: X'ten 3SAT'a giden ok çarpıyla çizilmiş; X ≤p 3SAT yalnızca X'in NP'de olduğunu gösterir. Altında bağımsız küme ≤p düğüm örtüsü: beş düğümlü çevrimde 2 koyu düğüm bağımsız küme, 3 açık düğüm örtü, 2 + 3 = 5; S bağımsız ⟺ V∖S örtü](assets/indirgeme-yonu.svg "Şekil 2 — İndirgemenin yönü: bilinen zor problemden yeni probleme")
...
Girdi boyutu, sayıların **kaç tane** olduğuyla değil, kaç **bit** tuttuğuyla ölçülür. n = 100 tane 64 bitlik sayı ve 64 bitlik bir hedef, toplam 6.464 bit, yani 808 bayt yer kaplar — ama hedef T, 2⁶⁴ ≈ 1,8 × 10¹⁹ kadar büyük olabilir ve tablo n·T ≈ 1,8 × 10²¹ hücre ister. Karmaşıklık makalesinde açtığımız "girdi boyutu sayı mı basamak mı" pini burada kapanır: **T girdinin uzunluğunda üsteldir**, dolayısıyla O(nT) girdi boyutunda polinom değildir.
...
Teorem şudur: **formül sağlanabilir, ancak ve ancak hiçbir x değişkeni kendi değiliyle aynı güçlü bağlı bileşende değilse.** "Ancak ve ancak"ın iki yönü ayrı ayrı savunulur. Bir yönü kısadır: aynı bileşende olmak, x'ten ¬x'e ve ¬x'ten x'e ima zinciri olması demektir, yani her iki atama da kendi kendini çürütür ve formül sağlanamaz. Öteki yön — çelişen bir bileşen yoksa bir atamanın gerçekten var olduğu — yapıcı olarak gösterilir ve yukarıdaki yoğunlaştırma DAG'ını kullanır: bileşenleri topolojik sıraya diz, sondan başa doğru dolaş ve değeri henüz verilmemiş her literale "doğru", değiline "yanlış" ata. Sedgewick ve Wayne atamanın bu topolojik sıralamadan okunduğunu kaydeder; bu atamanın hiçbir clause'u bozmadığının tam ispatı bu makalenin kapsamı dışında kalıyor. Kararı vermek Θ(n + m) sürer.
...
- Cormen, T. H., Leiserson, C. E., Rivest, R. L. & Stein, C. *Introduction to Algorithms*, dördüncü baskı, 34. bölüm (NP-Completeness) — polinom zamanlı doğrulama, NP-tamlık ve indirgenebilirlik tanımları; 34.4 (NP-completeness proofs) bölümünde devre sağlanabilirliğinden formül sağlanabilirliğine indirgemenin, devreyi doğrudan açmanın üstel büyüklükte formül verebileceği için her tele yeni bir değişken atayarak kurulması ve 3-CNF sağlanabilirliğinin NP-tamlığı. MIT Press, 2022. [Bağlantı](https://mitpress.mit.edu/9780262046305/introduction-to-algorithms/)
