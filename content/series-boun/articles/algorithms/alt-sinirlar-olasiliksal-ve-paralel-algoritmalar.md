...
tags:
  ...
  - is-ve-derinlik
content_hash: sha256:fabb42a22600234c119ca413a53f9759071ff52154c5aab163ad8d05af6e5361
...
**Arama.** n öğelik sıralı bir dizide bir değeri aramanın olası cevabı, değerin hangi konumda olduğu ya da hangi iki öğe arasına düştüğüdür: en az n + 1 farklı çıktı. Makinenin ikinci adımı doğrudan h ≥ lg(n + 1) verir ve ikili arama Θ(log n) karşılaştırmayla bu sınıra asimptotik olarak ulaşır. (6.006 notları aynı sonucu düğüm sayısı üzerinden ⌈lg(n + 1)⌉ − 1 biçiminde yazar; oradaki eksi bir, sınırı yaprak sayısı yerine ağacın toplam düğüm sayısı üzerinden kurmaktan gelir ve biraz daha gevşektir.) Hashing makalesinde bu argümanı sezgisel olarak kullanmıştık; formal hâli budur. Şekil 1 solda üç öğelik somut karar ağacını, sağda makinenin iki adımını ve sayısal sonuçları gösteriyor.

![Solda üç öğeyi sıralayan karar ağacı: kökte a : b, iç düğümlerde b : c ve a : c, dallar küçük ve büyük-eşit, vurgulu altı yaprak abc, acb, cab, bac, bca, cba; altında 6 yaprak, yükseklik 3, lg 6 ≈ 2,58 olduğundan en az 3 karşılaştırma. Sağ panel alt sınır: çıktıyı say, logaritmasını al. Kutuda L ≥ olası çıktı sayısı, L ≤ 2ʰ, dolayısıyla h ≥ lg L. Tabloda n = 3, 10, 20 için n!, lg n!, en az: 6, 2,58, 3; 3.628.800, 21,79, 22; yaklaşık 2,4 · 10¹⁸, 61,08, 62. Altında sıralamada h = Ω(n log n), aramada h ≥ lg(n + 1). Not: alt sınır modele aittir, sayma ve radix sıralamaları altına iner](assets/karar-agaci-alt-sinir.svg "Şekil 1 — Alt sınır makinesi: çıktı sayısını say, logaritmasını al")
...
**Monte Carlo örneği: çarpım denetleyicisi.** Elimizde n × n boyutlu A, B, C matrisleri var ve A × B = C olup olmadığını denetlemek istiyoruz. Çarpımı yeniden yapmak Strassen'le bile O(n^2,81) sürer. Freivalds'ın algoritması bunu Θ(n²)'ye indirir: rastgele bir ikili r vektörü seç ve A(Br) = Cr olup olmadığına bak. Üç matris-vektör çarpımı, her biri Θ(n²).

Analiz iki satırdır. A × B = C ise eşitlik her r için sağlanır; algoritma **kesinlikle** "evet" der. A × B ≠ C ise D = AB − C sıfır olmayan bir matristir ve d_ij ≠ 0 olan bir hücre vardır; o hücreyi kullanarak, hatayı kaçıran, yani Dr = 0 veren her r vektörünü, j'inci bileşenini çevirerek Dr′ ≠ 0 veren bir r′ vektörüyle birebir eşleyebiliriz. Eşleme birebir olduğu için hata veren r'lerin sayısı, yakalayan r'lerin sayısını aşamaz: **yanlış "evet" olasılığı en fazla 1/2**. Bağımsız k kez tekrarlarsan olasılık 2^(−k)'ye düşer; k = 20 ile milyonda birin altına iner. Bu sınırı 0/1 matrislerde mod 2 aritmetiğiyle deneysel olarak da ölçtüm: kasıtlı olarak tek bir hücresi bozulmuş 20.000 çarpımda yanlış "evet" oranı 0,4985 çıktı — sınıra tam oturuyor.
...
![İki panelli bir şema. Solda 8 sayının paralel toplanması: en altta sekiz kutu, üstünde dört, iki ve bir toplama düğümü; en soldaki kutudan köke giden koyu yol açıklıktır, yani en uzun bağımlılık zinciri. Altında iş T₁ = 7, açıklık T∞ = 3, paralellik ≈ 2,3; n = 10⁶ için iş ≈ 10⁶, açıklık 20, paralellik 50.000. Sağda üç kutu: iş yasası T_P ≥ T₁ / P, açıklık yasası T_P ≥ T∞ ve vurgulu kutuda açgözlü çizelgeleyici T_P ≤ T₁/P + T∞, optimalin en fazla 2 katı. Altında paralellik = T₁ / T∞, hızlanmanın tavanı; T₁ = 18, T∞ = 9, seri kesir 3/18 iken Amdahl 6 kat der, gerçek paralellik 2. Alttaki not: tavanı açıklık belirler](assets/is-ve-aciklik.svg "Şekil 2 — İki sayı, iki yasa: iş hızlanmanın tabanını, açıklık tavanını belirler")
