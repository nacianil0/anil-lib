...
- [2026-09-09] **Seri terim defteri, yeni bir ölçü çifti kurulmadan ÖNCE aranmalı.** Batch 20'de 83'e "precision/recall" için "kesinlik/geri çağırma" yazıldı; defterdeki 29 ("bulma oranı — 18 ve 21'deki geri çağırmayla karıştırılmaz") ve 45 ("kesinlik") satırları sonradan görülüp "geri çağırma" **kapsama** ile değiştirildi. Bedeli gövde + şekil + SVG alt metni + hash turunun tekrarı oldu.
- [2026-09-09] **İzole render kopyasında dev sunucusu derlenmiş sayfayı önbelleğe alıyor.** Ana ağaçta düzeltilen bir SVG'yi kopyaya `cp` ile senkronlamak yetmiyor; `preview_stop` + `preview_start` gerekiyor. (Katalog/frontmatter uyuşmazlığı uyarısı ayrı bir sorundu; bu, SVG için de geçerli.)
- [2026-09-09] **`.claude/launch.json` CRLF ile commit edilmiş.** Write aracıyla LF yazınca `git status` dosyayı "modified" gösteriyor; run sonunda `git checkout -- .claude/launch.json` ile geri al. Ayrıca Windows yollarını Python heredoc'una gömerken `D:\dev\...` yaz — tek `\` `unicodeescape` hatası ya da BEL karakteri veriyor.
...
- [2026-09-06] Seri şekilleri için **iki** ölçer gerekiyor: `tools/series/check-series-svg.cjs` (viewBox, sabit renk, yasak öge, font boyutu, 720'ye karşı taşma) ve `artifacts/b16-research/svgcheck-b16.py` (sütun başlangıçlarını çıkarır; her metni bir sonraki sütuna ve 700'e karşı 6,8 birim/karakter — kalın 7,4 — ile ölçer; alt payı bildirir, eşik 12). Batch 16'da ikincisi on bir sütun binmesini ve dokuz alt pay eksikliğini yazım aşamasında yakaladı ve PNG turunda yeni kusur çıkmadı; Batch 15'te aynı kusurlar yalnızca PNG'de görünmüştü.
- [2026-09-06] Yayıncı engelleri sabit: openai.com, iso.org, pnas.org, dl.acm.org ve academic.oup.com `urllib`'in varsayılan başlığına 403 döner; tarayıcı `User-Agent`'lı `curl -s -L -A "Mozilla/5.0 ... Chrome/128"` çalışır. RAND PDF'leri AES şifreli (boş parola) — `pip install cryptography` gerekir. Europe PMC 404 verirse `pmc.ncbi.nlm.nih.gov/articles/<PMCID>/` sayfası tam metni verir; ön baskılar için `osf.io/<id>/download`. Semantic Scholar toplu sorguda 429 döner, tek tek ve aralıklı sorulmalı.
...
### Sayfalı okuma çerçevesi (2026-09-06)
- Sayfalı modda `.reader-shell` tam `100dvh` ve `overflow: hidden`; `main > article.reader-area` sütun flex, `.prose-reader` ise `flex: 1 1 0%` + `min-height: 0` + `max-height: 52rem`. Yüksekliği artık `calc(100dvh - 11.5rem)` gibi sabit bir kroki değil, kalan boşluk belirliyor; header, resume-notice ve pager ne kadar yer kaplarsa sütunlar kalanı alıyor. `column-fill: auto` flex'in çözdüğü kesin yüksekliği kabul ediyor (Chromium'da ölçüldü: 1280×900 → 9 sayfa, 1440×720 → 10 sayfa, ikisinde de `documentElement.scrollHeight == innerHeight`).
- Sayfa çevirme tek yerden geçiyor: `commitPage` hem scroll komutunu veriyor hem `pageTargetRef`'i yazıyor. `updatePageState` komut yolda iken ara offset'leri yok sayıyor; `turnPage` de taban olarak canlı `scrollLeft` yerine komut edilen sayfayı alıyor. Bu olmadan smooth animasyon sırasında gelen ikinci hareket, okurun bulunduğu sayfaya geri komut veriyor ve hareket kayboluyor.
- Wheel/trackpad tek hareket = tek sayfa: `wheel-paging.ts` saf durum makinesi (eşik 50 px, hareket sonu 180 ms sessizlik). Momentum olayları 16 ms aralıklarla geldiği için sessizlik eşiğine takılmıyor; kilit yalnızca okur gerçekten durunca açılıyor. Dinleyici `main` üzerinde ve `{ passive: false }`; sidebar, header menüleri ve ayar paneli `main` dışında olduğu için kendi kaydırmalarını koruyor. Madde içi kendi kabında kayan öğeler (`.table-scroll`, `.series-figure-scroll`) eksen bazında kontrol ediliyor: yatay hareketi kap yutuyor, dikey hareket sayfayı çeviriyor.
...
- **Ön baskı başlığı ile yayımlanmış başlık farklı olabiliyor ve dizin araması bu yüzden boş dönüyor.** Batch 20'de Jamba ("Language Model" ↔ "Language Models"), Batch 21'de "The False Promise of Imitating Proprietary **LLMs**" ↔ ICLR 2024'te "…Proprietary **Language Models**". Bir künye konferans dizininde bulunamıyorsa "hakemsiz" sonucuna atlamadan önce OpenReview arama ucuna (`api2.openreview.net/notes/search?term=A+B&source=forum`) sorulmalı; `content.venue` alanı hakemli ↔ hakemsiz ayrımını tek başına veriyor.
- **ACL Anthology etkinlik sayfalarında `href` tırnaksız yazılıyor** (`href=/D16-1139/`). Tırnak bekleyen bir regex hiçbir şey bulamaz; `artifacts/b21-research/acl-b21.py` bunun için yazıldı. Sayfalar 3–12 MB, `aclanthology.org/events/acl-<yıl>/` ve `events/emnlp-<yıl>/`.
- **Okuyucu sayfasında SVG genişliği, sayfa yerleşimi oturmadan ölçülürse yanlış çıkıyor.** `navigate` sonrası ölçüm 609 px veriyordu; `await new Promise(r=>setTimeout(r,1800))` konunca 771 px (doğru değer) çıktı. Tarayıcı panosundan yapılan her geometri ölçümüne bekleme koy.
- **Bash aracında uzun Python heredoc'ları ayrıştırma hatası verebiliyor** ("unexpected EOF while looking for matching"). Uzun betikleri Write aracıyla `artifacts/<batch>-research/` altına yaz ve `python <dosya>` ile çalıştır.
...
- [2026-09-05] Tarayıcı panosunun `screenshot`'ına şekil doğrulaması için güvenme: Batch 15'te toplu `browser_batch` içinde "Image omitted", tek çağrıda 5 sn zaman aşımı ya da 1500×600 emülasyonun 800×323'lük kırpık köşesi döndü; Batch 14'ün "iki kez çağır" tarifi yetmedi. Şekil görüntüsünü `artifacts/b15-research/shots-b15.cjs` gibi bir Playwright betiğiyle PNG'ye yaz ve Read ile incele; DOM ölçümü panoda kalabilir.
- [2026-09-05] Playwright'ı `require('playwright')` ile çağırma; bu repoda yalnızca `@playwright/test` yüklü (`require('@playwright/test').chromium`). Dev sunucusuna karşı `waitUntil: 'networkidle'` yine 30 sn timeout verdi (reader-sync 503 döngüsü); `'load'` + `waitForSelector('main figure svg')` kullan.
- [2026-09-05] Düzeltme betiği + kopyaya `cp` + yeniden çekimi tek `&&` zincirine koyma: betik bir assert'te kırılınca `cp` atlandı, `;` ile bağlı çekim eski şekilleri gösterdi ve bir tur boşa gitti. Kopyaya taşımayı ayrı komutta yap ve `diff -rq` ile doğrula.
- [2026-09-05] SVG'de çok sütunlu serbest metin satırlarını sütun genişliğine karşı ölçmeden bırakma: `check-series-svg.cjs` yalnızca 720'ye karşı ölçer; 180 birimlik sütunda 13 birimlik satır ≤ 25 karakter (gerçek ≈ 6,8 birim/karakter), iki panelli şekilde sol panelin metni ayırıcı x'ini geçmez, 118 birimlik kutuya 20 karakterlik kalın başlık sığmaz, sağa dayalı komşu başlıklar arasında ≥ 15 birim bırak, köşegen/çizgi etiketi panel ayırıcısını kesmesin. Batch 15'te beş şekil yalnızca görüntüde yakalandı.
- [2026-09-05] Bash heredoc ile `.claude/launch.json` yazmayı tekrar denedim ve `\\` yine tek `\`'a inip JSON bozuldu (2026-09-04 kuralı). Python `json.dumps` ile yaz.
- [2026-09-05] Karar #145'i yalnızca şekil alt notuyla karşılanmış sayma: 65'te "kendi hesabımız" ibaresi gövdede yoktu, alt metinde vardı; `+1` fazında gövdeye eklendi ve hash yeniden senkronlandı. Kendi hesabı gövde cümlesine yazılır.

- [2026-09-06] Çok sütunlu bir SVG tablosunu yalnızca `check-series-svg.cjs` ile geçirme; o denetleyici sütuna binmeyi ve alt pay azlığını görmüyor. Her şekil dosyasından sonra `python artifacts/b16-research/svgcheck-b16.py <klasör>` çalıştır ve "sütuna biniyor" / "alt pay az" uyarılarını yazım aşamasında kapat.
- [2026-09-06] Şekil metnini kısaltırken markdown'daki alt metni güncellemeyi unutma: Batch 16'da 69-Şekil 3'ün etiketi "kimyasal, biyolojik, radyolojik ve nükleer" → "kimyasal ve biyolojik" yapılınca alt metin de aynı cümleyle güncellendi (alt metin gövde hash'ine girer, `sync-series-hashes --write` gerekir).
...
### İki serinin editoryal denetimi ve kalıcı kurallar (2026-09-03)
- **Karar:** Denetim tek ana agent (Fable 5.1) ile, workflow/subagent olmadan yürütüldü; kullanıcının görev metni bunu açıkça istedi ve ultracode hatırlatmasına rağmen kullanıcı talimatı üstün tutuldu. Sonuç: iki serinin 73 makalesi ve 186 şekli baştan sona okundu, yalnızca beş minimal gövde düzeltmesi gerekti; korpus kalitesi tutarlı biçimde yüksek.
- **Öğrenilen:** AI SOZLESME v2.0'daki "marka adı sayım değildir" kararı §1'de duruyordu ama editoryal kural olarak §2'ye yazılmamıştı; 1. makale bu yüzden "yüz makale" diyebilmişti. Ders: bir karar makale metnini bağlıyorsa §2 kurallarına da yazılmalı ve mekanik taramayla (grep) kapı hâline getirilmeli. Aynı ders terim defteri için: "gömme" yasağı defterde vardı, taraması yoktu (9 ve 27 kaçmıştı).
- **Öğrenilen:** SOZLESME §3'ün analoji kuralındaki örnek cümleler ("bu benzetme şurada bozulur") üretimde sabit kalıba dönüşmüş (21 kez "Benzetmenin bozulduğu yer şurası… biçimsel karşılığı ise şudur"). Kural içerik gereği + kalıp yasağı olarak normalize edildi (v2.1 §2/§3); geriye dönük üslup düzeltmesi yapılmadı, yalnızca 7 ve 9'daki on satır içi tekrarlar çeşitlendirildi.
- **Öğrenilen:** BOUN makaleleri önceki makalelere numarayla değil konu adıyla gönderme yapıyor ("hash makalesinde"); bu yazılı kural değildi ama seriyi yeniden sıralamaya dayanıklı kılıyor. BOUN SOZLESME v1.1 §2'ye yazıldı. BOUN'da ileri numaralı gönderme yok; AI'da tüm ileri göndermeler vaat defteriyle eşleşti (tarama betiği: scratchpad `fwdrefs.py` mantığı — `(\d{1,3})(?:\\)?\.\s*makale`).
...
- [2026-09-05] Batch 14 (59–62): Faz 6 kapandı, Faz 7 `safety-and-evaluation` ile açıldı (karar #142); kohort iki kategoriye yayıldı ve `reading-list-groups.test.ts` değişmeden 482 test geçti. Terim: "devir / devretme" (deferral) başlıkta; "reddederek öğrenme" (rejection learning, 59) ↔ "reddetme / ret" (refusal, 62) ayrımı; "hizalama" 61'den itibaren 11'in anlamında (#143); kendi hesabımızla verilen sayılar gövdede işaretlenir (#145). Kaynak: yayın başlığı arXiv'den farklı olan makaleler DBLP aramasında yanlış hit verir (RouteLLM, Skalse, IRD/CIRL) → papers.nips.cc yıl dizini ya da ICLR proceedings hash'i; Semantic Scholar `openAccessPdf` eski dergi makalelerinin açık kopyasını verir (`curl -k`); DBLP `ee` 72 başlıkta yavaş ama takılmaz. SVG: gösterge ile sütun başlığı aynı y bandına konmaz (61-Şekil 3 binmesi denetleyiciden kaçtı, ekran görüntüsünde çıktı). Tarayıcı: JS kaplayıcıdan sonra `screenshot` ilk çağrıda 5 sn zaman aşımına düşer, ikinci çağrı gelir; yerel dosyaya açılmış sekme `navigate` etmez; sekme sınırı var. Ultracode açıkken de workflow/subagent kullanılmadı (kullanıcı talimatı). Kararlar #142–#147 YOL-HARITASI'nda.

- [2026-09-05] Batch 15 (63–66): Faz 7'nin gövdesi, tamamı `safety-and-evaluation`; 61–66 tek öbek, `reading-list-groups.test.ts` değişmeden 495 test. "jailbreak" başlıkta korundu (#108 ölçütü; karar #148); "Constitutional AI" özel ad; "düşmanca" (adversarial) yeni; "danışmanlık" (64) ↔ 59'un "danışman" rolü ve "cevaplama oranı" (65) ↔ 33'ün "kapsama"sı aynı sözcük–başka nesne notlarıyla deftere girdi (#149). 64 koordinatı ödendi, yeni koordinat açılmadı; sıradaki bağlayıcı koordinat 69 (#153). Kaynak: 84 kalemin 30'u hakemsiz — Faz 7'de oran yükseliyor, işaretlenerek kullanıldı (#152); Europe PMC `fullTextXML` Nature OA tam metnini verir; PMLR cilt dizini DBLP 503 verince yedek; COLM kabul sayfası challenge döndü. Render: DOM ölçümü panoda, şekil görüntüleri Playwright betiğiyle (`shots-b15.cjs`) — pano `screenshot`'ı bu run'da hiç güvenilir olmadı; beş SVG binmesi yalnızca PNG'de çıktı. Ultracode açıkken de workflow/subagent kullanılmadı (kullanıcı talimatı). Kararlar #148–#153 YOL-HARITASI'nda.
...
### Sayfalı okuma deneyimi: çerçeve, hareket ve kontrol yerleşimi (2026-09-06)
