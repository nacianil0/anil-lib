...
type BatchGroup = {
  batch: number;
  articleCount: number;
  categories: { category: Category; articles: ArticleDescriptor[] }[];
};

function groupByBatchAndCategory(articles: ArticleDescriptor[]): BatchGroup[] {
  ...
  for (const article of articles) {
    ...
    lastBatch.articleCount++;
    
    let lastCategory = lastBatch.categories[lastBatch.categories.length - 1];
    if (!lastCategory || lastCategory.category !== article.category) {
      lastCategory = { category: article.category, articles: [] };
      ...
      <div className="flex flex-col gap-8">
        {batches.map((batchGroup) => (
          <div key={`batch-${batchGroup.batch}`} className="flex flex-col gap-5">
            <h2 className="pl-7 font-sans text-xs font-medium text-text-muted">
              <span className="mb-3 block w-full border-t border-border" aria-hidden="true" />
              {UI.batchHeader(batchGroup.batch, batchGroup.articleCount)}
            </h2>
            
            {batchGroup.categories.map((group) => (
              <section key={group.category} aria-labelledby={`${idPrefix}-batch-${batchGroup.batch}-${group.category}`}>
                <h3
                  id={`${idPrefix}-batch-${batchGroup.batch}-${group.category}`}
